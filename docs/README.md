...
## 得意なこと
- 仕様が完全に固まっていない状態でも動くものをサクッと作って議論のテーブルに乗せる
...
- フロントエンド担当として施策検討・新機能開発を主に担当
  - 必要に応じてノーコードツール（KARTE や Google Optimize)を導入し AB テストを実施
...
- リファクタ
  - ボタンのコンポーネントにてサイズ指定の有無で使用箇所の css を切り分けないといけない実装があったため size Props のみの指定で済むようにリファクタ
