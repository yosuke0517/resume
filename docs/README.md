...
# no.7
- 【案件概要】: タスク管理ツール新規開発
- 期間: 半年（2020/04~2021/09）※週12,3hほどの稼働です
- 規模: 7名ほど
- 役割: 開発メンバー（バックエンド）
### 開発手法
- アジャイル
...
| 案件名            | 使用技術 | 備考 |
|-----------------------|--------------|--------------------------|
| タスク管理ツール新規開発|     Node.js      | websocketによるAPI開発|
|              |       mongodb      | タスク情報などの永続化     | |

### 実績・取り組み
- websocketを用いたAPIを実装
- 技術選定にて、書き込みが多く発生することとJSONのオブジェクトがそのまま格納できることからMongoDBを選定したことがアジャイルと相性が良く今のところ成功している
...
### 実績・取り組み
- フロントエンド担当として新機能開発を主に担当。必要に応じてコーディングだけでなく、ノーコードツール（KARTE)やGAなどを駆使した施策も実施。
- Google Optimizeを使用してABテストを実施。
- 2週間スプリントにて安定して1スプリントに1施策をリリース。
- 固定されたレビュワーがいなく、相互レビューを行う文化だったので[レビューポイント](https://yosuke0517.github.io/reviewPoints/) をgithub管理してgithub pagesで公開し精度の高いレビューを安定的にこなせるようにした
...
- 期間: 1年（2020/02~2021/04）※期間はno.4と重なってます
- 規模: 2名
...
- 退職してフリーランスになる関係でα版リリースまでは見届けられなかったものの2人で通常の業務後に開発を進めていた

---

# no.4
- 【案件概要】: 携帯電話機種変更補助アプリ(新規開発）(SIer)
- 期間: 1年（2020/02~2021/04）
- 規模: 15名程度
- 役割: 開発メンバー（フロントエンド）
### 開発手法
- アジャイル
...
| 案件名            | 使用技術 | 備考 |
|-----------------------|--------------|--------------------------|
| 携帯電話機種変更補助アプリ(新規開発）|       Java 8      | 別チームが開発のためほとんど触っていない|
|              |       MySQL      | -     | |
|              |       nginx      | リバースプロキシとして使用     | |
|              |       Nuxt.js(Vue.js)           | 10個ほどの画面中で難易度の高い3ページを担当|

### 実績・取り組み
- 新規開発Webアプリの開発メンバーとしてアサイン。役職はないが、他エンジニアへ知見を教えたりレビューするポジション
- いわゆる紙芝居のような形式で手続きが進んでいくイメージのアプリ。ルーティングを工夫する必要があり、3種類のルーティングに分けて管理した。
- 初めてのアジャイルで要求に応じてstoreやコンポーネントがガンガン変わっていき少し苦労したがなんとか対応した。

---

# no.3
- 【案件概要】: 予算管理アプリの新規開発(SIer)
- 期間: 1年（2019/01~2020/01）
- 規模: 20名程度
- 役割: 開発メンバー（主にフロントエンド）
### 開発手法
- ウォーターフォール
...
|              |       Nuxt.js(Vue.js)           | 8画面中２画面その他多数のコンポーネント開発|
|              |       TypeScript           | |

### 実績・取り組み
- 新規開発Webアプリの開発メンバーとしてアサイン。初めてのNuxt.jsだったが、早期にキャッチアップし期待に答えた。
- Storybookでのコンポーネントのカタログ管理を学んだ。
- jestでのユニットテスト
...
- 不具合例)
  ...
  - ■対策：SessionStrageを使用してブラウザに データを持たせて値を保持する JavaScriptを追加することで解消。
