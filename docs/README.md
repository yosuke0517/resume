...
## 得意なこと
- 抽象⇄具体
  - 抽象的な課題（画面遷移率が低いなど）から具体的課題を発見し、仮説・検証を高速に繰り返す
    - 動くものをサクッと作って議論のテーブルに乗せABテストをする
    - 上記を繰り返して新しい課題や適切な施策を生む
- UI/UX的な観点からの施策提案
  - 例: 賃貸検索アプリ→入力項目が多い→離脱と繋がっていた
  - ↑ ユーザの中の抽象的な「住みたい部屋」を具体化するようなUIを模索
  - ↑「日当たりのいい部屋に住みたい」のようにあえて抽象的な項目を作りそれを選ぶと南向き・最上階が OR 条件で設定されるように施策提案
