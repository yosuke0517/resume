...
| CSS（ほぼSCSS）        |       A      |       4Y      | 設計（命名）はBEMがほとんど、Tailwind CSSはプライベートのみ                               |
...
- ミドルウェアの知見はあまりないながらも以下の理由からMongodbをプライベート以外で初めて技術選定した...
  - 軽いRead/Writeが大量にある
...
| 案件名            | 使用技術 | 備考 |
|-----------------------|--------------|--------------------------|
| 予算管理アプリケーション（新規開発）|     PHP      | APIサーバ：補助的な開発・軽微な調査のみ|
...
### 実績・取り組み
- 会社員時代の社内のプロジェクトの一環で自社サービス開発として企画・デザインの検討・フロントエンドのデプロイ（配信）など主にフロントエンド周りを担当
- 企画もゼロの全くの新規開発。スモールスタートでいきたいとは言いつつもあれもこれもと盛り込んでしまい「やっぱこれやめたい」となったときの心理的ダメージが大きかった。
- リリース直前で退職してフリーランスになったので少し悔しかった
...
- 
  - 親ルート1
    - 子ルート1
    - 子ルート2（顧客の状態や入力次第で次に遷移する親ルート先が変わる）
  - 親ルートxx
    - 子ルート1
    - 子ルート2
  - 親ルートyy
